...
namespace
{
QPixmap makeDaylightMapIcon(bool on)
{
	QPixmap pix(160, 160);
...
StelModule* DaylightMapStelPluginInterface::getStelModule() const
{
	return new DaylightMap();
}

StelPluginInfo DaylightMapStelPluginInterface::getPluginInfo() const
{
	Q_INIT_RESOURCE(DaylightMap);

	StelPluginInfo info;
	info.id = "DaylightMap";
	info.displayedName = N_("Daylight Map");
	info.authors = "Atque";
	...
	info.description = N_("This plugin shows Earth daylight and twilight, including the sunrise-sunset "
	                       ...
	                       "subpoint markers, and rise/set isolines.");
	info.version = DAYLIGHTMAP_PLUGIN_VERSION;
	info.license = DAYLIGHTMAP_PLUGIN_LICENSE;
	return info;
}

DaylightMap::DaylightMap()
	: mainWindow(new DaylightMapDialog())
...
{
	if (!conf->childGroups().contains("DaylightMap"))
	...
	if (b)
	{
		if (toolbarButton == Q_NULLPTR)
		{
			toolbarButton = new StelButton(Q_NULLPTR,
			                               makeDaylightMapIcon(true),
			                               makeDaylightMapIcon(false),
