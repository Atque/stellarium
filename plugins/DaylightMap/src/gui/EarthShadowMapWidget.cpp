	...
	, cachedJD(0.)
{
	...
	const QString mapPath = StelFileMgr::findFile("textures/earthmap.png", StelFileMgr::File);
	if (!mapPath.isEmpty())
		earthMap.load(mapPath);
}
...
void EarthShadowMapWidget::paintEvent(QPaintEvent* event)
{
	...
	{
		painter.drawPixmap(mapRect.topLeft(), baseMapCache);
	}
	else
	{
	...
	const QString legend = q_("Terminator") + QStringLiteral("  |  ") +
	                       q_("Civil") + QStringLiteral("  |  ") +
	                       q_("Nautical") + QStringLiteral("  |  ") +
	                       q_("Astronomical");
	painter.drawText(QRectF(mapRect.left(), mapRect.bottom() + 4,
	                        mapRect.width(), height() - mapRect.bottom() - 4),
	                 Qt::AlignLeft | Qt::AlignVCenter, legend);
}
...
EarthShadowMapWidget::BodyPoint EarthShadowMapWidget::computeSubPoint(bool sun) const
{
	...
	const double gastDeg = get_apparent_sidereal_time(core->getJD(), core->getJDE());
...
{
	return QPointF(mapRect.left() + (normalizeLongitudeDeg(longitudeDeg - mapCenterLongitudeDeg) + 180.0) / 360.0 * mapRect.width(),
...
void EarthShadowMapWidget::rebuildSceneCache(const QSize& mapSize)
{
	...
	const double jd = core ? core->getJD() : 0.;
	const bool timeChanged = qAbs(jd - cachedJD) > 1.0e-9;
	const QSize overlaySize = mapSize;

	if (!timeChanged && cachedOverlaySize == overlaySize && !twilightOverlayCache.isNull() && !contourCache.isNull())
		return;
...
void EarthShadowMapWidget::rebuildBaseMapCache(const QSize& mapSize)
{
	...
	if (earthSvg.isValid())
	{
		earthSvg.render(&painter, QRectF(QPointF(0., 0.), QSizeF(mapSize)));
	}
	else if (!earthMap.isNull())
	{
		painter.drawPixmap(QRect(QPoint(0, 0), mapSize),
		                   earthMap.scaled(mapSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
	}
}

void EarthShadowMapWidget::rebuildTwilightOverlayCache(const QSize& cacheSize, const BodyPoint& sun)
{
	twilightOverlayCache = QImage(cacheSize, QImage::Format_ARGB32_Premultiplied);
	...
	for (int y = 0; y < cacheSize.height(); ++y)
	{
		...
		{
			const double lon = normalizeLongitudeDeg(mapCenterLongitudeDeg - 180.0 +
			                                         (static_cast<double>(x) + 0.5) * 360.0 / cacheSize.width());
			const double alt = solarAltitudeDeg(lat, lon, sun);

			if (alt < astronomicalTwilightAltitudeDeg)
				line[x] = qRgba(0, 0, 0, 153);
			else if (alt < nauticalTwilightAltitudeDeg)
				line[x] = qRgba(0, 0, 0, 115);
			else if (alt < civilTwilightAltitudeDeg)
				line[x] = qRgba(0, 0, 0, 77);
			else if (alt < sunriseSunsetAltitudeDeg)
				line[x] = qRgba(0, 0, 0, 38);
		}
	}
}
                                               ...
                                               double altitudeDeg, const QPen& pen) const
{
	...
	auto valueAt = [&](int ix, int iy) {
		const double lon = normalizeLongitudeDeg(mapCenterLongitudeDeg - 180.0 +
		                                         static_cast<double>(ix) * 360.0 / columns);
